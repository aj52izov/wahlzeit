...
public class CoordinateId {
	 ...
	 * 
	 */
	protected static CoordinateId[] ids = new CoordinateId[BUFFER_SIZE_INCREMENT];
	...
	public static synchronized void setCurrentIdFromInt(int id) {
		currentId = id;
		ids = new CoordinateId[currentId + BUFFER_SIZE_INCREMENT];
		ids[0] = NULL_ID;
	}
	...
	public static synchronized int getNextIdAsInt() {
		...
		if (currentId >= ids.length) {
			CoordinateId[] nids = new CoordinateId[currentId + BUFFER_SIZE_INCREMENT];
	...
	public static CoordinateId getIdFromInt(int id) {
		...
		CoordinateId result = ids[id];
		if (result == null) {
			synchronized(ids) {
				result = ids[id];
				if (result == null) {
					result = new CoordinateId(id);
					ids[id] = result;
				}
			}
		}
	 ...
	 * 
	 */
	public static CoordinateId getIdFromString(String id) {
	 ...
	 * 
	 */
	public static CoordinateId getNextId() {
	 ...
	 * 
	 */
	public static CoordinateId getRandomId() {
	 ...
	 * 
	 */
	protected CoordinateId(int myValue) {
		value = myValue;
	...
	public boolean equals(Object o) {
		...
		CoordinateId pid = (CoordinateId) o;
		return isEqual(pid);
	}
	 ...
	 * 
	 */
	public boolean isEqual(CoordinateId other) {
